...
The object that store values may be:

- a structure (including empty structure)
- a list (including empty list)
- a boolean value (true or false)

Structures and lists must be JSON-compatible since hooks get the parameters in the JSON files (see [using values in hook](#using-values-in-hook)).

Boolean values are for enable and disable the module; this case is handled by [modules discovery](LIFECYCLE.md#modules-discovery) process.
